...
static void probe_real_size(GL *gl, struct ra_format *fmt) {
  ...
  bool is_la =
      gl_fmt->format == GL_LUMINANCE || gl_fmt->format == GL_LUMINANCE_ALPHA;
  if (is_la && gl->es)
  ...
  for (int i = 0; i < fmt->num_components; i++) {
    const GLenum pnames[] = {
        ...
        GL_TEXTURE_ALPHA_SIZE, GL_TEXTURE_LUMINANCE_SIZE, GL_TEXTURE_ALPHA_SIZE,
    };
...
static int ra_init_gl(struct ra *ra, GL *gl) {
  ...
  for (int n = 0; gl_formats[n].internal_format; n++) {
    ...
    *fmt = (struct ra_format){
        ...
        .renderable = (gl_fmt->flags & F_CR) && (gl->mpgl_caps & MPGL_CAP_FB),
...
static void gl_destroy(struct ra *ra) { talloc_free(ra->priv); }
...
static struct ra_tex *gl_tex_create_blank(struct ra *ra,
  ...
  case 1:
    tex_gl->target = GL_TEXTURE_1D;
    break;
  case 2:
    tex_gl->target = GL_TEXTURE_2D;
    break;
  case 3:
    tex_gl->target = GL_TEXTURE_3D;
    break;
  default:
  ...
  if (params->downloadable &&
      !(params->dimensions == 2 && params->format->renderable)) {
...
static struct ra_tex *gl_tex_create(struct ra *ra,
  ...
  switch (params->dimensions) {
  case 1:
    gl->TexImage1D(tex_gl->target, 0, tex_gl->internal_format, params->w, 0,
...
// this function.
// Some features are unsupported, e.g. setting params->initial_data or
// render_dst.
...
struct ra_tex *ra_create_wrapped_fb(struct ra *ra, GLuint gl_fbo, int w,
                                    int h) {
...
void ra_gl_get_raw_tex(struct ra *ra, struct ra_tex *tex, GLuint *out_texture,
                       GLenum *out_target) {
  struct ra_tex_gl *tex_gl = tex->priv;
...
bool ra_is_gl(struct ra *ra) { return ra->fns == &ra_fns_gl; }
...
static bool gl_tex_upload(struct ra *ra,
  ...
  case 1:
    gl->TexImage1D(tex_gl->target, 0, tex_gl->internal_format, tex->params.w, 0,
  ...
  case 2: {
    ...
    if (params->rc)
      rc = *params->rc;
    gl_upload_tex(gl, tex_gl->target, tex_gl->format, tex_gl->type, src,
                  params->stride, rc.x0, rc.y0, rc.x1 - rc.x0, rc.y1 - rc.y0);
    break;
  }
  case 3:
    gl->PixelStorei(GL_UNPACK_ALIGNMENT, 1);
...
static bool gl_tex_download(struct ra *ra,
...
static struct ra_buf *gl_buf_create(struct ra *ra,
  ...
  case RA_BUF_TYPE_TEX_UPLOAD:
    buf_gl->target = GL_PIXEL_UNPACK_BUFFER;
    break;
  case RA_BUF_TYPE_SHADER_STORAGE:
    buf_gl->target = GL_SHADER_STORAGE_BUFFER;
    break;
  case RA_BUF_TYPE_UNIFORM:
    buf_gl->target = GL_UNIFORM_BUFFER;
    break;
  default:
    abort();
  };
...
static void gl_clear(struct ra *ra, struct ra_tex *dst, float color[4],
  ...
  gl->Scissor(scissor->x0, scissor->y0, scissor->x1 - scissor->x0,
...
static void printShaderToFile(const char *source) {
  static int count = 1;
  char filename[80];
  sprintf(filename, "shader%d.txt", count++);
...
static void compile_attach_shader(struct ra *ra, GLuint program, GLenum type,
                                  const char *source, bool *ok) {
  GL *gl = ra_gl_get(ra);

  printShaderToFile(source);
...
static GLuint compile_program(struct ra *ra,
                              const struct ra_renderpass_params *p) {
  GL *gl = ra_gl_get(ra);
...
static struct ra_renderpass *
gl_renderpass_create(struct ra *ra, const struct ra_renderpass_params *params) {
  GL *gl = ra_gl_get(ra);
...
static void update_uniform(struct ra *ra, struct ra_renderpass *pass,
  ...
  case RA_VARTYPE_FLOAT: {
    ...
    if (input->dim_m == 1) {
      switch (input->dim_v) {
      case 1:
        gl->Uniform1f(loc, f[0]);
        break;
      case 2:
        gl->Uniform2f(loc, f[0], f[1]);
        break;
      case 3:
        gl->Uniform3f(loc, f[0], f[1], f[2]);
        break;
      case 4:
        gl->Uniform4f(loc, f[0], f[1], f[2], f[3]);
        break;
      default:
        MP_ASSERT_UNREACHABLE();
      }
...
static void disable_binding(struct ra *ra, struct ra_renderpass *pass,
  ...
  case RA_VARTYPE_TEX: {
    ...
    if (input->type == RA_VARTYPE_TEX) {
      ...
      gl->BindImageTexture(input->binding, 0, 0, GL_FALSE, 0, GL_WRITE_ONLY,
...
static void gl_renderpass_run(struct ra *ra,
  ...
  case RA_RENDERPASS_TYPE_COMPUTE: {
    gl->DispatchCompute(params->compute_groups[0], params->compute_groups[1],
